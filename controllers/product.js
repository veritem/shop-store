...
exports.getProduct = async (req, res) => {
	...
	res.status(200).json({ count: findProduct.length, success: true, data: findProduct })
}
exports.addProduct = asyncHandler(async (req, res, next) => {
	const { name, description, price } = req.body

	if (!name | !description | !price) return next(new ErrorResponse('All filed are required', 403))
	...
	const newProduct = await Product.create({ name, description, price, productImage: file.name })
